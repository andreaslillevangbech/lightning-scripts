...
includeByMeasure measure init = snd . M.foldlWithKey includeIf (init, mempty)
  where
    includeIf acc@(n, out) nodeId nodePaths
...
-- NOTE: What if there is a node that cant be reached??? What is the shortest path
shortestPath ::
     [Payee] -> Map NodeId (Map NodeId Path) -> Map NodeId (Map NodeId Path)
shortestPath payees = includeByMeasure weightedLength (Just (Down NoPath))
  where
    weightedLength nodePaths =
      ...
      case mapM (`M.lookup` nodePaths) $ node <$> payees of
        Nothing -> Nothing
        Just paths ->
          Just . Down . PathLength . sum $ zipWith pathLength (weight <$> payees) paths
...
-- NOTE: Why does fold left vs fold right matter here??
